/* =========================================================
   ...
   Depende del importmap "three" declarado en el HTML.
   ========================================================= */
...
if (mount) {
  ...
  const R = 1;                       // radio del planeta

  /* ---------- Nodos: ciudades reales (lat, lon) ---------- */
  const CITIES = [
  ...
  // Textura de resplandor (punto suave) generada en canvas
  ...
  // Textura de la Luna generada en canvas (cráteres + mares oscuros)
  function moonTexture() {
    const c = document.createElement('canvas'); c.width = 512; c.height = 256;
    const g = c.getContext('2d');
    g.fillStyle = '#8f8f96'; g.fillRect(0, 0, 512, 256);
    for (let i = 0; i < 900; i++) {                    // ruido de cráteres
    ...
    for (let i = 0; i < 7; i++) {                      // "mares" oscuros
  ...
  if (renderer) {
    ...
    // Luces: "sol" direccional (crea el terminador día/noche) + relleno frío
    ...
    /* ---------- Planeta ---------- */
    const spin = new THREE.Group();                    // rota por inercia + arrastre
    scene.add(spin);

    const earthMat = new THREE.MeshStandardMaterial({
      color: 0x2a3a55, roughness: 1, metalness: 0,
      emissive: 0xffffff, emissiveIntensity: 0.5       // las "luces de ciudades" brillan solas
    });
    const earth = new THREE.Mesh(new THREE.SphereGeometry(R, 64, 64), earthMat);
    spin.add(earth);

    // Textura nocturna de la Tierra (con fallback si la CDN falla)
    ...
    // Anillos pulsantes (pool reutilizable)
    const rings = [];
    ...
    /* ---------- Conexiones (paquetes viajando por arcos) ---------- */
    const MAX_LINKS = 16;
    const links = [];
    ...
    function newLink() {
      ...
      let b = a;
      for (let tries = 0; tries < 6; tries++) { b = CITIES[(Math.random() * CITIES.length) | 0]; if (b !== a) break; }
      const va = llToVec3(a.lat, a.lon, R), vb = llToVec3(b.lat, b.lon, R);
      if (va.angleTo(vb) < 0.5) return;                // arcos demasiado cortos, descartar
      ...
      const line = new THREE.Line(
        ...
        new THREE.LineBasicMaterial(linkMatBase)
      );
      ...
      links.push({ line, packet, curve, t: 0, speed: 0.16 + Math.random() * 0.2,
                   lat: Math.round(20 + Math.random() * 160), mb: 5 + Math.round(Math.random() * 90), a, b });
      pulse(va);
    }

    /* ---------- Luna ---------- */
    const moonOrbit = new THREE.Group();               // plano inclinado de la órbita
    moonOrbit.rotation.set(0.45, 0, 0.28);
    scene.add(moonOrbit);
    ...
    let spawnT = 0, hudT = 0, running = true, raf;

    function update(dt, now) {
      idle += dt;
      if (idle > 2.5 && !REDUCE) spin.rotation.y += dt * 0.06;      // giro automático suave
      moonPivot.rotation.y += dt * 0.22;                            // órbita lunar
    ...
    new IntersectionObserver(es => {
      es.forEach(en => {
        ...
        if (running) clock.getDelta();               // evitar salto de dt al reanudar
      });
    }, { threshold: 0.05 }).observe(mount);

    /* ---------- Arranque ---------- */
    if (REDUCE) {
      for (let i = 0; i < 8; i++) { newLink(); links.forEach(l => { l.t = 0.5; l.packet.position.copy(l.curve.getPoint(0.5)); }); }
      renderer.render(scene, camera);                // una sola vez, sin bucle
    } else {
      loop();
    }
  }
